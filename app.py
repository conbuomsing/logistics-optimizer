...
with tab1:
    with st.form("logistics_form"):
        ...
        with col1:
            cargo_volume = st.number_input("Tổng thể tích hàng (m³)", min_value=0.1)
    ...
    if submitted:
        ...
        # Biểu đồ phân bổ
if not df_results.empty:  # Kiểm tra DataFrame có dữ liệu không
    fig = go.Figure(data=[
...
with tab3:
    ...
    fig_forecast.update_layout(title='Dự báo nhu cầu vận chuyển')
